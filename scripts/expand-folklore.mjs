...
const MANUAL_CT = {
  ...
  'kr-bari-gongju': 'myth', 'kr-mago-halmi': 'myth', 'kr-hwanung': 'myth',
  'kr-jeseoksin': 'myth', 'kr-chilseongsin': 'myth', 'kr-jowangsin': 'myth',
  'kr-eeopsin': 'myth', 'kr-teojusin': 'myth', 'kr-seongju': 'myth',
  'kr-ondal': 'legend', 'kr-pyeonggang-gongju': 'legend',
  // Korea - folktale
  ...
  'kr-simcheong': 'folktale', 'kr-chunhyang': 'folktale', 'kr-kongjwi': 'folktale',
  'kr-patjwi': 'folktale', 'kr-haenim-dalnim': 'folktale', 'kr-namu-kkun': 'folktale',
  'kr-gyeonu-jingnyeo': 'folktale', 'kr-tokki': 'folktale',
  'kr-heungbu': 'folktale', 'kr-nolbu': 'folktale',
  // Japan
...
const NEW_BY_COUNTRY = {
  ...
  KR: [
    ...
    // ──── 한국 신화 인물/영웅 확장 ────
    mk('kr','Bari Gongju','바리공주','Divine Heroine',5,
      '버림받은 일곱째 공주. 죽어가는 부모를 살리기 위해 저승까지 가서 생명수를 구해온다. 한국 무속신화의 대표작으로, 바리공주는 죽은 자의 영혼을 저승으로 인도하는 무조신(巫祖神)이 되었다.',
      ['저승 여행','생명수 획득','영혼 인도'],['인간적 한계','시련의 길'],
      ['abandoned princess','underworld journey','life water','shaman ancestor'],'바리공주 서사무가','myth'),
    mk('kr','Mago Halmi','마고할미','Creation Goddess',4,
      '한국 창세신화의 거대한 여신. 산과 섬을 만들고, 바다를 갈라 지형을 형성했다. 제주 한라산을 베개삼아 잤다는 전설이 있으며, 각 지역마다 마고할미의 지형 창조 설화가 전해진다.',
      ['지형 창조','거대한 몸','대지의 힘'],['없음'],
      ['giant goddess','mountain maker','creation','primordial'],'한국 창세신화','myth'),
    mk('kr','Hwanung','환웅','Sky God',4,
      '환인(천제)의 아들. 천부인 세 개를 가지고 3000명의 무리를 이끌고 태백산 신단수 아래 신시(神市)를 열었다. 바람·비·구름의 신을 거느리며 인간 세상을 다스렸고, 웅녀와 혼인하여 단군을 낳았다.',
      ['천부인(天符印)','바람·비·구름 지배','신시 통치'],['천상으로의 귀환'],
      ['sky god','divine seal','sacred tree','heavenly prince'],'한국 건국신화','myth'),
    mk('kr','Simcheong','심청','Filial Heroine',4,
      '앞 못 보는 아버지의 눈을 뜨게 하기 위해 인당수에 제물로 몸을 던진 효녀. 용궁에서 연꽃에 싸여 다시 세상에 나왔고, 왕비가 되어 아버지와 재회한다. 한국 효(孝)의 상징.',
      ['자기 희생','용궁 생환','연꽃 환생'],['지나친 효심으로 인한 위험'],
      ['filial daughter','lotus flower','sacrifice','ocean','blind father'],'심청전','folktale'),
    mk('kr','Chunhyang','춘향','Faithful Heroine',3,
      '남원 퇴기 월매의 딸. 이몽룡과 사랑에 빠지지만 변학도의 수청 요구를 거부하여 옥에 갇힌다. 암행어사가 된 이몽룡이 돌아와 구해낸다. 조선시대 신분 차별에 저항한 절개의 상징.',
      ['절개','불굴의 의지','사랑의 힘'],['신분 제약','권력자의 횡포'],
      ['faithful woman','prison','magistrate','love story'],'춘향전','folktale'),
    mk('kr','Kongjwi','콩쥐','Virtuous Heroine',3,
      '계모와 이복언니 팥쥐의 학대를 받는 착한 소녀. 동물들과 초자연적 도움으로 시련을 극복하고 행복을 찾는다. 한국판 신데렐라로 불리며, 권선징악의 교훈을 담은 대표 민담.',
      ['동물과의 교감','초자연적 도움','인내'],['계모의 학대','육체적 시련'],
      ['virtuous girl','stepmother','animals help','korean cinderella'],'콩쥐팥쥐전','folktale'),
    mk('kr','Patjwi','팥쥐','Villain',6,
      '콩쥐의 이복언니. 계모와 함께 콩쥐를 학대하고 그녀의 행복을 빼앗으려 한다. 결국 악행의 대가를 치르게 되는 전형적인 악역으로, 한국 민담 권선징악의 상징.',
      ['속임수','시기','모함'],['진실 발각','인과응보'],
      ['evil stepsister','jealousy','schemer','villain'],'콩쥐팥쥐전','folktale'),
    mk('kr','Haenim-Dalnim','해님달님 남매','Celestial Siblings',3,
      '호랑이에게 쫓기던 오누이가 하늘에서 내려온 동아줄을 타고 올라가 오빠는 해가, 여동생은 달이 되었다. 한국 대표 민담으로, 위기 극복과 천상 변신을 다룬다.',
      ['천상 승천','해와 달로 변신','동아줄'],['호랑이의 추격'],
      ['sun and moon','siblings','rope from heaven','tiger chase'],'해님달님','folktale'),
    mk('kr','Namu-kkun','나무꾼','Mortal Hero',3,
      '사슴의 도움으로 선녀의 날개옷을 숨겨 아내로 맞은 나무꾼. 아이 셋을 낳기 전에 날개옷을 돌려주어 선녀가 하늘로 떠난다. 후에 수탉을 타고 하늘로 올라가지만 향수병으로 다시 지상에 떨어진다.',
      ['순박함','사슴의 조언'],['약속 위반','향수병'],
      ['woodcutter','deer advisor','hidden robe','celestial ascent'],'선녀와 나무꾼','folktale'),

    // ──── 무속 신격 ────
    mk('kr','Jeseoksin','제석신','Deity',3,
      '삼신과 함께 출산과 아이의 성장을 관장하는 무속 신. 불교의 제석천에서 유래했으나, 무속에서는 곡물과 풍요의 신으로 변화했다. 제석본풀이에 등장하는 핵심 신격.',
      ['풍요 부여','아이 수호','곡식 관장'],['없음'],
      ['grain deity','fertility','shamanic','buddhist origin'],'제석본풀이','myth'),
    mk('kr','Chilseongsin','칠성신','Deity',3,
      '북두칠성을 관장하는 무속 신. 인간의 수명과 운명을 결정하며, 자녀의 건강과 장수를 기원할 때 모시는 신이다. 장독대에 정화수를 떠놓고 비는 대상이기도 하다.',
      ['수명 관장','운명 결정','별의 가호'],['없음'],
      ['seven stars','longevity','fate','north star deity'],'한국 무속','myth'),
    mk('kr','Jowangsin','조왕신','Hearth Deity',2,
      '부엌을 관장하는 가신(家神). 아궁이와 불을 지키며, 가정의 평화와 안녕을 돌본다. 매년 섣달그믐에 하늘에 올라가 그 집안의 일을 옥황상제에게 보고한다고 전해진다.',
      ['가정 수호','화재 방지','하늘에 보고'],['부엌이 더러우면 노함'],
      ['hearth god','kitchen','fire guardian','household deity'],'한국 무속/가신신앙','myth'),
    mk('kr','Eeopsin','업신','Guardian Spirit',3,
      '집안의 재물과 복을 관장하는 가신. 구렁이, 족제비, 두꺼비 등의 형태로 집에 깃든다. 업이 떠나면 집안이 기울고, 업을 잘 모시면 부귀영화를 누린다고 믿었다.',
      ['재물 수호','복 부여','동물 변신'],['박대하면 떠남'],
      ['household snake','wealth guardian','prosperity','animal spirit'],'한국 가신신앙','myth'),
    mk('kr','Teojusin','터주신','Land Deity',2,
      '집터를 지키는 가신. 뒤뜰에 짚으로 만든 터주가리(작은 단지)를 놓아 모신다. 집안의 안전과 터의 기운을 관장하며, 이사할 때 반드시 고사를 지내 터주신에게 인사해야 했다.',
      ['터 수호','집안 안전','기운 관장'],['고사를 지내지 않으면 노함'],
      ['land guardian','house site','straw jar','foundation deity'],'한국 가신신앙','myth'),
    mk('kr','Seongju','성주신','House Deity',3,
      '집의 대들보를 관장하는 최고 가신. 집을 새로 지을 때 성주굿을 하여 모셔야 하며, 가장의 운명과 집안 전체의 흥망성쇠를 좌우한다. 성주풀이 서사무가에 그 내력이 전해진다.',
      ['가옥 수호','가장 운명 관장','흥망성쇠 결정'],['집이 허물어지면 약해짐'],
      ['house beam deity','master guardian','ridgepole','household chief'],'성주풀이','myth'),

    // ──── 추가 설화 존재 ────
    mk('kr','Gyeonu-Jingnyeo','견우직녀','Celestial Lovers',3,
      '은하수 양쪽에 떨어진 견우(소치는 별)와 직녀(베짜는 별). 칠월 칠석에만 까마귀와 까치가 오작교를 놓아 만날 수 있다. 동아시아 공통 설화이나, 한국에서는 칠석 풍습과 깊이 연결된다.',
      ['별의 사랑','오작교 건너기'],['1년에 하루만 만남','은하수 격리'],
      ['cowherd','weaver girl','magpie bridge','milky way','qixi'],'칠석 설화','folktale'),
    mk('kr','Tokki','토끼 (별주부전)','Trickster Hero',2,
      '별주부(자라)의 꾀에 속아 용궁에 갔지만, 간을 육지에 두고 왔다는 기지로 위기를 벗어난 지혜로운 토끼. 약자가 강자를 이기는 한국 민담의 전형을 보여준다.',
      ['기지','말재주','위기 탈출'],['순진함(처음 속아넘어감)'],
      ['clever rabbit','trickster','dragon palace','wit'],'별주부전(토끼전)','folktale'),
    mk('kr','Ondal','온달','Folk Hero',4,
      '바보로 불리던 고구려의 평민. 평강공주와 혼인한 뒤 무예를 익혀 전쟁에서 대공을 세운 장군이 되었다. 아신산성 전투에서 전사한 비극적 영웅으로, 신분 극복의 상징.',
      ['무예','용맹','신분 초월'],['전쟁의 위험'],
      ['fool general','commoner hero','warrior','princess'],'온달전(삼국사기)','legend'),
    mk('kr','Pyeonggang Gongju','평강공주','Royal Heroine',3,
      '울보 공주로 불리다 아버지의 농담대로 바보 온달에게 시집간 고구려 공주. 온달을 훌륭한 장군으로 키워낸 현명한 여인으로, 교육과 내조의 힘을 보여주는 설화.',
      ['지혜','교육','헌신'],['왕실 추방'],
      ['crying princess','wise wife','goguryeo','education'],'온달전(삼국사기)','legend'),
    mk('kr','Heungbu','흥부','Virtuous Hero',2,
      '가난하지만 착한 동생. 다친 제비를 고쳐주었더니 제비가 박씨를 물어다 주었고, 그 박에서 금은보화가 쏟아졌다. 놀부와 대비되는 선량함의 상징으로, 권선징악의 대표 민담.',
      ['선량함','동물 구호','복 받음'],['가난','형의 구박'],
      ['kind brother','swallow','gourd','treasure'],'흥부전','folktale'),
    mk('kr','Nolbu','놀부','Greedy Villain',5,
      '부유하지만 욕심 많은 형. 흥부를 내쫓고, 흥부의 성공을 보고 일부러 제비 다리를 부러뜨렸다가 박에서 도깨비가 나와 재산을 잃었다. 탐욕의 인과응보를 보여주는 전형적 악역.',
      ['탐욕','모방 시도'],['인과응보','도깨비의 징벌'],
      ['greedy brother','broken swallow','gourd punishment','villain'],'흥부전','folktale'),
  ],
